...
class ClipCocoDataset(Dataset):
    ...
    def pad_tokens(self, item: int):
        ...
        if padding > 0:
            tokens = torch.cat(
                (tokens, torch.full((padding,), self.padding_id, dtype=torch.int64) - 1)
            )
...
class TransformerLayer(nn.Module):
    ...
    def __init__(
        ...
        self.attn = MultiHeadAttention_own(
...
class ClipCaptionModel(nn.Module):
    def get_dummy_token(self, batch_size: int, device: torch.device) -> torch.Tensor:
        ...
        return (
            torch.ones(batch_size, self.prefix_length, dtype=torch.int64, device=device)
            * -100
        )
    ...
    def forward(
        ...
        prefix_projections = self.clip_project(prefix).view(
            -1, self.prefix_length, self.model_embedding_size
        )
    ...
    def __init__(
        ...
        self.model_embedding_size = self.embedding_func.weight.shape[1]
        print(self.model_embedding_size)

        if mapping_type == MappingType.MLP:
            self.clip_project = MLP(
                (
                    prefix_size,
                    (self.model_embedding_size * prefix_length) // 2,
...
def load_model(
    model_name, config_path: str, epoch_or_latest: Union[str, int] = "_latest"
):
    ...
    if args.only_prefix:
        model = ClipCaptionPrefix(args.prefix_length, model_name)
    else:
        model = ClipCaptionModel(args.prefix_length, model_name)
    ...
    for epoch in range(nb_epochs):
        ...
        for idx, (tokens, mask, prefix) in enumerate(train_dataloader):
            ...
            logits = outputs.logits[
                :, dataset.prefix_length - 1 : -1
            ]  # we want the predictions of what is happening after the image
            ...
            accelerator.log({"loss": loss, "lr": scheduler.get_last_lr()[0]})
...
def main():
    ...
    parser.add_argument(
        "--mapping_type", type=str, default="transformer", help="mlp/transformer"
    )
    ...
    dataset = ClipCocoDataset(
        args.data, prefix_length, model_name, normalize_prefix=args.normalize_prefix
    )
    ...
    accelerator.init_trackers(
        project="clip_caption",
        config={
            "mapping_architecture": args.mapping_type,
            "language_model": model_name,
            "prefix_length": prefix_length,
            "total_epochs": nb_epochs,
            "dataset": args.data,
            "clip_prefix_length": args.prefix_length_clip,
        ...
        init_kwargs={
            "wandb": {
                "entity": "coleman-and-friends",
                "save_code": True,
                "tags": ["french", "bloom", "transformer"],
                "resume": "auto",
                "group": "french",
            },
        },
